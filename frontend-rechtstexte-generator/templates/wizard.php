...
<div class="frg-wizard" data-frg-wizard>
	...
	<form class="frg-form" data-frg-form>
		...
		<section class="frg-step" data-step="3">
			...
			<div class="frg-grid frg-grid--2">
				<label class="frg-toggle frg-grid__full"><input type="checkbox" name="has_editorial_content" value="1" <?php checked( ! empty( $data['has_editorial_content'] ) ); ?>><span><?php esc_html_e( 'Die Website enthält journalistisch-redaktionell gestaltete Angebote (§ 18 Abs. 2 MStV)', 'frontend-rechtstexte-generator' ); ?></span></label>
				<p class="frg-grid__full frg-field-note"><?php esc_html_e( 'Nur auswählen, wenn die Website tatsächlich journalistisch-redaktionelle Inhalte anbietet, etwa einen redaktionell geführten News-, Magazin- oder Ratgeberbereich. Normale Leistungs-, Referenz-, Über-uns- und Kontaktseiten reichen hierfür nicht automatisch aus.', 'frontend-rechtstexte-generator' ); ?></p>
				<div class="frg-grid__full frg-conditional-fields" data-frg-conditional="has_editorial_content">
					<div class="frg-grid frg-grid--2">
						<label><span><?php esc_html_e( 'Name der verantwortlichen Person *', 'frontend-rechtstexte-generator' ); ?></span><input type="text" name="responsible_name" value="<?php echo esc_attr( $data['responsible_name'] ?? '' ); ?>" data-frg-conditional-required></label>
						<label><span><?php esc_html_e( 'Anschrift der verantwortlichen Person *', 'frontend-rechtstexte-generator' ); ?></span><textarea name="responsible_address" rows="4" data-frg-conditional-required><?php echo esc_textarea( $data['responsible_address'] ?? '' ); ?></textarea></label>
					</div>
				</div>
				...
				<div class="frg-grid__full frg-conditional-fields" data-frg-conditional="has_professional_info">
					<div class="frg-grid frg-grid--2">
						...
						<label><span><?php esc_html_e( 'Berufsbezeichnung', 'frontend-rechtstexte-generator' ); ?></span><input type="text" name="professional_title" value="<?php echo esc_attr( $data['professional_title'] ?? '' ); ?>" placeholder="<?php echo esc_attr__( 'z. B. Anlagenmechaniker für Sanitär-, Heizungs- und Klimatechnik', 'frontend-rechtstexte-generator' ); ?>"></label>
						<label class="frg-toggle frg-grid__full"><input type="checkbox" name="has_professional_award_location" value="1" <?php checked( ! empty( $data['has_professional_award_location'] ) ); ?>><span><?php esc_html_e( 'Staat/Land der Verleihung im Impressum ausgeben', 'frontend-rechtstexte-generator' ); ?></span></label>
						<p class="frg-grid__full frg-field-note"><?php esc_html_e( 'Nur aktivieren, wenn diese Angabe für die konkrete reglementierte Berufsbezeichnung erforderlich und sachlich zutreffend ist. Bei gewöhnlichen Ausbildungsberufen ist sie nicht automatisch notwendig.', 'frontend-rechtstexte-generator' ); ?></p>
						<div class="frg-grid__full frg-conditional-fields" data-frg-conditional="has_professional_award_location">
							<label><span><?php esc_html_e( 'Staat/Land der Verleihung', 'frontend-rechtstexte-generator' ); ?></span><input type="text" name="professional_awarded_in" value="<?php echo esc_attr( $data['professional_awarded_in'] ?? '' ); ?>"></label>
						</div>
						...
						<label class="frg-grid__full"><span><?php esc_html_e( 'Berufsrechtliche Regelungen', 'frontend-rechtstexte-generator' ); ?></span><textarea name="professional_rules" rows="4" placeholder="<?php echo esc_attr__( 'z. B. Handwerksordnung (HwO): https://www.gesetze-im-internet.de/hwo/', 'frontend-rechtstexte-generator' ); ?>"><?php echo esc_textarea( $data['professional_rules'] ?? '' ); ?></textarea><small><?php esc_html_e( 'Nur konkret einschlägige Vorschriften mit genauer Bezeichnung und möglichst direktem Link angeben. Ein pauschaler Link auf die Startseite einer Kammer wird nicht veröffentlicht.', 'frontend-rechtstexte-generator' ); ?></small></label>
					</div>
				</div>
		...
		<section class="frg-step" data-step="4">
			...
			<div class="frg-feature-group frg-feature-group--section">
				...
				<div class="frg-grid frg-grid--2">
					...
					<label><span><?php esc_html_e( 'Serverstandort *', 'frontend-rechtstexte-generator' ); ?></span><select required name="server_location"><?php foreach ( array( '' => __( 'Bitte wählen', 'frontend-rechtstexte-generator' ), 'Deutschland' => __( 'Deutschland (bestätigt)', 'frontend-rechtstexte-generator' ), 'EU' => __( 'EU/EWR (bestätigt)', 'frontend-rechtstexte-generator' ), 'Drittland' => __( 'Drittland', 'frontend-rechtstexte-generator' ), 'unbekannt' => __( 'Unbekannt', 'frontend-rechtstexte-generator' ) ) as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $data['server_location'] ?? $privacy_defaults['server_location'], $value ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select><small><?php esc_html_e( 'Deutschland oder EU/EWR nur wählen, wenn der tatsächliche Serverstandort durch den Anbieter oder die Vertragsunterlagen bestätigt ist. Andernfalls „Unbekannt“ wählen.', 'frontend-rechtstexte-generator' ); ?></small></label>
					...
					<label><span><?php esc_html_e( 'Server-Infrastruktur-Anbieter', 'frontend-rechtstexte-generator' ); ?></span><input type="text" name="server_infrastructure_provider" value="<?php echo esc_attr( $data['server_infrastructure_provider'] ?? $privacy_defaults['server_infrastructure_provider'] ); ?>" placeholder="<?php echo esc_attr__( 'z. B. netcup GmbH', 'frontend-rechtstexte-generator' ); ?>"></label>
					<label><span><?php esc_html_e( 'Art der Server-Infrastruktur', 'frontend-rechtstexte-generator' ); ?></span><input type="text" name="server_infrastructure_type" value="<?php echo esc_attr( $data['server_infrastructure_type'] ?? $privacy_defaults['server_infrastructure_type'] ); ?>" placeholder="<?php echo esc_attr__( 'z. B. virtuelle Server (VServer)', 'frontend-rechtstexte-generator' ); ?>"></label>
					<label class="frg-grid__full"><span><?php esc_html_e( 'Anschrift Server-Infrastruktur-Anbieter', 'frontend-rechtstexte-generator' ); ?></span><textarea name="server_infrastructure_address" rows="4" placeholder="<?php echo esc_attr__( "Firmenname\nStraße und Hausnummer\nPLZ Ort\nLand", 'frontend-rechtstexte-generator' ); ?>"><?php echo esc_textarea( $data['server_infrastructure_address'] ?? $privacy_defaults['server_infrastructure_address'] ); ?></textarea></label>
				</div>
			</div>
			<div class="frg-feature-group frg-feature-group--section">
				<div class="frg-feature-group__header">
			...
			<div class="frg-feature-group frg-feature-group--section">
				...
				<div class="frg-grid frg-grid--2">
					<label class="frg-grid__full"><span><?php esc_html_e( 'Bundesland des Verantwortlichen', 'frontend-rechtstexte-generator' ); ?></span>
						<select name="privacy_supervisory_authority_state" data-frg-authority-state>
							<option value=""><?php esc_html_e( 'Bitte Bundesland wählen', 'frontend-rechtstexte-generator' ); ?></option>
							<?php foreach ( FRG_Authorities::get_state_labels() as $state_key => $state_label ) : $authority = FRG_Authorities::get_by_state()[ $state_key ]; ?>
								<option value="<?php echo esc_attr( $state_key ); ?>" data-authority-name="<?php echo esc_attr( $authority['name'] ); ?>" data-authority-address="<?php echo esc_attr( $authority['address'] ); ?>" data-authority-url="<?php echo esc_url( $authority['url'] ); ?>" <?php selected( $data['privacy_supervisory_authority_state'] ?? '', $state_key ); ?>><?php echo esc_html( $state_label ); ?></option>
							<?php endforeach; ?>
						</select>
						<small><?php esc_html_e( 'Die Auswahl übernimmt die allgemeine Landesaufsicht für nichtöffentliche Stellen. Bestehende Feldinhalte werden dabei ersetzt und können anschließend manuell angepasst werden.', 'frontend-rechtstexte-generator' ); ?></small>
					</label>
					<label><span><?php esc_html_e( 'Name der Aufsichtsbehörde', 'frontend-rechtstexte-generator' ); ?></span><input type="text" name="privacy_supervisory_authority_name" data-frg-authority-name value="<?php echo esc_attr( $data['privacy_supervisory_authority_name'] ?? '' ); ?>" placeholder="<?php echo esc_attr__( 'z. B. Der Landesbeauftragte für den Datenschutz Niedersachsen', 'frontend-rechtstexte-generator' ); ?>"></label>
					<label><span><?php esc_html_e( 'Website der Aufsichtsbehörde', 'frontend-rechtstexte-generator' ); ?></span><input type="url" name="privacy_supervisory_authority_url" data-frg-authority-url value="<?php echo esc_attr( $data['privacy_supervisory_authority_url'] ?? '' ); ?>" placeholder="https://www.lfd.niedersachsen.de/"></label>
					<label class="frg-grid__full"><span><?php esc_html_e( 'Anschrift der Aufsichtsbehörde', 'frontend-rechtstexte-generator' ); ?></span><textarea name="privacy_supervisory_authority_address" data-frg-authority-address rows="4" placeholder="<?php echo esc_attr__( "Prinzenstraße 5\n30159 Hannover\nDeutschland", 'frontend-rechtstexte-generator' ); ?>"><?php echo esc_textarea( $data['privacy_supervisory_authority_address'] ?? '' ); ?></textarea></label>
					<p class="frg-grid__full frg-field-note"><?php esc_html_e( 'Bitte prüfen Sie die Zuständigkeit vor Veröffentlichung. Für öffentliche Stellen, Kirchen sowie Presse und Rundfunk können andere Aufsichtsstellen zuständig sein.', 'frontend-rechtstexte-generator' ); ?></p>
				</div>
			</div>
		</section>
		...
		<section class="frg-step" data-step="5">
			...
			$feature_groups = array(
				...
				array(
					...
					'items'       => array(
						'user_registration' => __( 'Öffentlich angebotene Benutzerregistrierung', 'frontend-rechtstexte-generator' ),
						'login_area'        => __( 'Kunden-/Nutzer-Login-Bereich (nicht wp-admin)', 'frontend-rechtstexte-generator' ),
